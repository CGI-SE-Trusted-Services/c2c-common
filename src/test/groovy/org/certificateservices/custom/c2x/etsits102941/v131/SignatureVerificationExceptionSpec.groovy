...
/**
 * Unit tests for SignatureVerificationException
 */
class SignatureVerificationExceptionSpec extends Specification {
    ...
    def "Verify that constructor populates fields correctly"(){
        ...
        when:
        def e1 = new SignatureVerificationException("SomeMessage", key)
        then:
        e1.message == "SomeMessage"
        e1.getSecretKey() == key

        when:
        def e2 = new SignatureVerificationException("SomeMessage", cause ,key, requestHash, receiver)
        then:
